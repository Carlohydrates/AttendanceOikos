        ...
        if ($request->hasFile('file')) {
            ...
            for ($i = 1; $i < count($sheetData); $i++) {
                ...
                Students::create([
                    ...
                    'mname'=> $sheetData[$i][2],
                    'lname'=> $sheetData[$i][1],
                    'extension'=> $sheetData[$i][3],
                    'level'=> (int)$sheetData[$i][5],
                    'section'=> $sheetData[$i][6],
                    'fetcher'=> $sheetData[$i][4],
                    'enroll_status'=> $sheetData[$i][7],
                    'bday'=> $sheetData[$i][8],
                    'age'=>(int)$sheetData[$i][9],
                    'date_enrolled'=>$date_enrolled,
                    'address'=> $sheetData[$i][10],
                    'city'=> $sheetData[$i][11],
                    'region'=> $sheetData[$i][12],
                    'postal_code'=> (int)$sheetData[$i][13],
                    'country'=> $sheetData[$i][14],
                    'nationality'=> $sheetData[$i][15],
                    'sex'=> $sheetData[$i][16],
                    'telephone_number'=> $sheetData[$i][17],
                    'mobile_number'=> $sheetData[$i][18]
                ]);
                User::create([
                    'id'=>$student_id,
                    'email'=>$email,
